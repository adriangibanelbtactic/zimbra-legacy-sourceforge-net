...
if(ZaResource.myXModel && ZaResource.myXModel.items) {
		ZaResource.myXModel.items.push({id:ZaSamAccount.A_isSpecialNTAccount,type:_NUMBER_, defaultValue:0,ref:ZaSamAccount.A_isSpecialNTAccount});
		ZaResource.myXModel.items.push({id:ZaSamAccount.A_sambaDomainSID,type:_STRING_,ref:ZaSamAccount.A_sambaDomainSID});				
				
		ZaResource.myXModel.items.push({id:ZaSamAccount.A_sambaSID,type:_STRING_,ref:"attrs/"+ZaSamAccount.A_sambaSID, required:true});			
		ZaResource.myXModel.items.push({id:ZaSamAccount.A_sambaAcctFlags,type:_STRING_, defaultValue:0,ref:"attrs/"+ZaSamAccount.A_sambaAcctFlags});	
		ZaResource.myXModel.items.push({id:ZaSamAccount.A_sambaBadPasswordCount,type:_NUMBER_, defaultValue:0,ref:"attrs/"+ZaSamAccount.A_sambaBadPasswordCount});
		ZaResource.myXModel.items.push({id:ZaSamAccount.A_sambaBadPasswordTime,type:_NUMBER_, defaultValue:0,ref:"attrs/"+ZaSamAccount.A_sambaBadPasswordTime});		
		ZaResource.myXModel.items.push({id:ZaSamAccount.A_sambaDomainName,type:_STRING_,ref:"attrs/"+ZaSamAccount.A_sambaDomainName});		
		ZaResource.myXModel.items.push({id:ZaSamAccount.A_sambaHomeDrive,type:_STRING_,ref:"attrs/"+ZaSamAccount.A_sambaHomeDrive});		
		ZaResource.myXModel.items.push({id:ZaSamAccount.A_sambaHomePath,type:_STRING_,ref:"attrs/"+ZaSamAccount.A_sambaHomePath});			
		ZaResource.myXModel.items.push({id:ZaSamAccount.A_sambaKickoffTime,type:_NUMBER_,ref:"attrs/"+ZaSamAccount.A_sambaKickoffTime});		
		ZaResource.myXModel.items.push({id:ZaSamAccount.A_sambaLMPassword,type:_STRING_,ref:"attrs/"+ZaSamAccount.A_sambaLMPassword});				
		ZaResource.myXModel.items.push({id:ZaSamAccount.A_sambaLogoffTime,type:_NUMBER_,ref:"attrs/"+ZaSamAccount.A_sambaLogoffTime});						
		ZaResource.myXModel.items.push({id:ZaSamAccount.A_sambaLogonHours,type:_STRING_,ref:"attrs/"+ZaSamAccount.A_sambaLogonHours});								
		ZaResource.myXModel.items.push({id:ZaSamAccount.A_sambaLogonScript,type:_STRING_,ref:"attrs/"+ZaSamAccount.A_sambaLogonScript});			
		ZaResource.myXModel.items.push({id:ZaSamAccount.A_sambaLogonTime,type:_NUMBER_,ref:"attrs/"+ZaSamAccount.A_sambaLogonTime});		
		ZaResource.myXModel.items.push({id:ZaSamAccount.A_sambaMungedDial,type:_STRING_,ref:"attrs/"+ZaSamAccount.A_sambaMungedDial});
		ZaResource.myXModel.items.push({id:ZaSamAccount.A_sambaNTPassword,type:_STRING_,ref:"attrs/"+ZaSamAccount.A_sambaNTPassword});				
		ZaResource.myXModel.items.push({id:ZaSamAccount.A_sambaPasswordHistory,type:_STRING_,ref:"attrs/"+ZaSamAccount.A_sambaPasswordHistory});		
		ZaResource.myXModel.items.push({id:ZaSamAccount.A_sambaPrimaryGroupSID,type:_STRING_,ref:"attrs/"+ZaSamAccount.A_sambaPrimaryGroupSID});		
		ZaResource.myXModel.items.push({id:ZaSamAccount.A_sambaProfilePath,type:_STRING_,ref:"attrs/"+ZaSamAccount.A_sambaProfilePath});		
		ZaResource.myXModel.items.push({id:ZaSamAccount.A_sambaPwdCanChange,type:_NUMBER_,ref:"attrs/"+ZaSamAccount.A_sambaPwdCanChange});		
		ZaResource.myXModel.items.push({id:ZaSamAccount.A_sambaPwdLastSet,type:_NUMBER_,ref:"attrs/"+ZaSamAccount.A_sambaPwdLastSet});		
		ZaResource.myXModel.items.push({id:ZaSamAccount.A_sambaPwdMustChange,type:_NUMBER_,ref:"attrs/"+ZaSamAccount.A_sambaPwdMustChange});			
		ZaResource.myXModel.items.push({id:ZaSamAccount.A_sambaUserWorkstations,type:_STRING_,ref:"attrs/"+ZaSamAccount.A_sambaUserWorkstations});
}
...
if(ZaTabView.XFormModifiers["ZaResourceXFormView"]) {
	ZaSamAccount.ResourceXFormModifier= function (xFormObject) {
		var cnt = xFormObject.items.length;
		var i = 0;
		for(i = 0; i <cnt; i++) {
			if(xFormObject.items[i].type=="switch") 
		...
		cnt = xFormObject.items[i].items.length;
		var sambaTabIx = ++ZaResourceXFormView.TAB_INDEX;
	...
	ZaTabView.XFormModifiers["ZaResourceXFormView"].push(ZaSamAccount.ResourceXFormModifier);	
}

if(ZaXDialog.XFormModifiers["ZaNewResourceXWizard"]) {
	
	ZaSamAccount.ResourceXWizModifier= function (xFormObject) {
		var stepCounter = this.stepChoices.length;			
		ZaNewResourceXWizard.SAMBA_ACC_STEP = stepCounter+1;			
	
		this.stepChoices.push({value:ZaNewResourceXWizard.SAMBA_ACC_STEP, label:"Samba Account"});
		this._lastStep = this.stepChoices.length;
		...
		var sambaAccountStep={type:_CASE_, numCols:1, relevant:"instance[ZaModel.currentStep] == ZaNewResourceXWizard.SAMBA_ACC_STEP",		
					items: [
						{type:_ZAWIZGROUP_, 
							items:[
								{ref:ZaSamAccount.A_sambaDomainSID, type:_OSELECT1_, editable:false,choices:this._app.getSambaDomainSIDListChoices(), msgName:"Samba domain",label:"Samba domain", labelLocation:_LEFT_,
	...
	ZaXDialog.XFormModifiers["ZaNewResourceXWizard"].push(ZaSamAccount.ResourceXWizModifier);	
}
...
if(ZaItem.loadMethods["ZaResource"]) {
	ZaItem.loadMethods["ZaResource"].push(ZaSamAccount.loadMethod);
}
...
ZaSamAccount.createResourceMethod =
function(tmpObj, account, app) {
	if(tmpObj.attrs[ZaAccount.A_password] && tmpObj.attrs[ZaAccount.A_password].length > 0) {
...
if(ZaItem.createMethods["ZaResource"]) {
	ZaItem.createMethods["ZaResource"].push(ZaSamAccount.createResourceMethod);
}
